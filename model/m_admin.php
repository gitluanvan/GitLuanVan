...
//thực hiện câu lệnh truy vấn
		...
		public function InsertDataLKH($id_SP,$table){
			$sql="INSERT INTO $table values(null,'$id_SP') ";
			return $this->execute($sql);
		}
		//update status
		...
		public function getBinhLuan($id_user){
			...
			$sql="SELECT binhluan.NoiDung as NoiDung, binhluan.id_SanPham as id_SanPham FROM binhluan INNER JOIN traloi ON binhluan.id_BinhLuan = traloi.id_BinhLuan WHERE traloi.id_User = '$id_user' AND traloi.NoiDungTraLoi =''";
			$this->execute($sql);
			if($this->num_rows()==0){
				$data=array();
			}
